...
colnames(all[[i]]) = sub("juxta_", "", colnames(all[[i]]))
colnames(all[[i]]) = sub("ct", "", colnames(all[[i]]))
colnames(all[[i]]) = sub("ct", "",colnames(all[[i]]))
colnames(all[[i]]) = sub("(.*)_(.*)", "\\2_\\1", colnames(all[[i]]))

```
Make 16 features for GIOTTO
...
```
 ...
 Chenge feature vector order in Misty and SEA
```{r}
i = 136

for(i in grep("SEA_bi_", names(all))){
  colnames(all[[i]]) = sub("X", "", colnames(all[[i]]))
  colnames(all[[i]]) = sub("(.*)_(.*)", "\\2_\\1", colnames(all[[i]]))
  }

for(i in grep("Misty", names(all))){
  colnames(all[[i]]) = sub("juxta_", "", colnames(all[[i]]))
  colnames(all[[i]]) = sub("ct", "", colnames(all[[i]]))
  colnames(all[[i]]) = sub("ct", "",colnames(all[[i]]))
  colnames(all[[i]]) = sub("(.*)_(.*)", "\\2_\\1", colnames(all[[i]]))
...
colnames(all[[149]]) = sub("X", "", colnames(all[[150]]))
pheatmap(all[[149]], treeheight_row = 0, treeheight_col = 0, show_rownames = FALSE)
dev.off()
...
```
run random forest on all the datasets
```{r}
list_rf = list()
stat = list()
...
sig_matrix <- as.matrix(all[[i]])
#sig_matrix = sig_matrix[-c(101:200),]
#labels <- rep(c("ran","self06","self045"), each=100)
...
# Print the cross-validation results, commented out for now
#list_rf[[data[i]]] = cv_rf
misclass_rate = mean(get.misclass.error(labels, cv_rf$class.oob))

# accuracy and F1 score
conf <- confusionMatrix(data=cv_rf$class.oob, reference = cv_rf$class)
#Display results
F1 = mean(conf$byClass[7])
# FDR is 1 minus PPV
FDR = 1 - mean(conf$byClass[3])
#Precision = mean(conf$byClass[,5])
stat[[i]] = c(F1, FDR)
}
...
#saveRDS(result_list, "./../../results_4ct_asym_0.2grid_self/Misty_cross/Misty_F1_correct_order.rds")
...
# change rownames for SEA
for(i in grep("SEA_bi_", names(features))){
  for (x in 1:100){
  rownames(features[[i]][[x]]) = sub("X", "", rownames(features[[i]][[x]]))
  rownames(features[[i]][[x]]) = sub("(.*)_(.*)", "\\2_\\1", rownames(features[[i]][[x]]))
  match = gsub("`", "",rownames(features[[100]][[1]]))
  vec <- features[[i]][[x]][order(match(match, gsub("`", "",rownames(features[[i]][[x]])))), ]
  features[[i]][[x]]$Overall = vec
  rownames(features[[i]][[x]]) <- sub("X", "",rownames(features[[100]][[1]]))
  }
}

#for(i in grep("Misty", names(features))){
#  for (x in 1:100){
#  rownames(features[[i]][[x]]) = sub("juxta_", "", rownames(features[[i]][[x]]))
#  rownames(features[[i]][[x]]) = sub("ct", "", rownames(features[[i]][[x]]))
#  rownames(features[[i]][[x]]) = sub("ct", "", rownames(features[[i]][[x]]))
#  rownames(features[[i]][[x]]) = sub("`", "", rownames(features[[i]][[x]]))
#  rownames(features[[i]][[x]]) = sub("`", "", rownames(features[[i]][[x]]))
#  rownames(features[[i]][[x]]) = sub("(.*)_(.*)", "\\2_\\1", rownames(features[[i]][[x]]))
#  #match = gsub("`", "",rownames(features[[1]][[1]]))
#  #vec <- features[[i]][[x]][order(match(match, gsub("`", "",rownames(features[[i]][[x]])))), ]
#  #features[[i]][[x]]$Overall = vec
#  #rownames(features[[i]][[x]]) <- sub("X", "",rownames(features[[100]][[1]]))
#  }
...
for (i in seq_along(features)) {
  for (x in 1:100) {
      # Order row names of dataset1 based on row names of dataset2
      #features_ord[[i]] <- features[[i]][[x]][order(match(rownames(features[[i]][[x]]), rownames(features[[i]][[1]]))), ]
      
      # Order row names of dataset1 based on row names of dataset2
      ordered_rownames <- rownames(features[[i]][[1]])

      # Reorder rows of dataset1 using the ordered row names
      features[[i]][[x]] <- features[[i]][[x]][ordered_rownames, , drop = FALSE]
  }
}
...
pheatmap(features[[14]])

#reorder to ground-truth order
vector = sub("(.*)_(.*)", "\\2_\\1", rownames(features[[1]]))
vector = sub("`","", vector)
vector = sub("`", "",vector)
...
for (i in 1:length(features)){
  rownames(features[[i]]) = sub("`","", rownames(features[[i]]))
  rownames(features[[i]]) = sub("`", "",rownames(features[[i]]))
  features[[i]] <- features[[i]][order(match(rownames(features[[i]]), vector)), , drop = FALSE]
}
...
ggplot(cosine_similarities, aes(x = factor(abundance_ct_0), y = cosine_similarity, color = comparison, group = interaction(comparison, abundance_ct_0))) +
  geom_boxplot() +
  facet_grid(. ~ tool) +
  theme_test() +
  theme(axis.text.x = element_text(size = 9, angle = 90, vjust = 0.5, hjust = 1)) +
