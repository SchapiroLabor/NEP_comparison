...
```{r}
source("utils.R")
# define folder where all files from simulated cohorts are stored
csv_dir = "./../../results_4ct_asym/"
```

Load in all different result files and ensure correct ordering of the interaction direction
```{r}
...
# order columns the same way for all datasets
...
Create defining group columns within datasets for binary classification
```{r}
df_list = list()
...
Naming convention for every comparison: Tool_x_vs_y
```{r}
# prepare lists
df_comb = list()
split_string <- strsplit(data, "4ct")

# now loop through every combination of abundances and preferences to compare all tools withtin their abundances
for (i in 1:length(all)){
...
Create comparison of interaction score distributions
```{r}
...
foreach(i = 1:length(all), .combine = c, .packages = c("caret","randomForest")) %dopar% {
  ...
  for (x in 1:num_iterations){
    ...
    # Define the model, CURRENTLY CHANGE TO GLM
    model <- caret::train(group_index ~ ., data = train, method = "glm", trControl = trainControl(method = "cv", number = 5))
...
#df = rbind(df, base)
# save df
# Assuming your dataframe is named df_all_avg_clean
write.csv(df, "./../../RF_comparison_sym.csv", row.names = FALSE)
...
#dev.off()
#ggsave(file = "./../../results_4ct_asym/asym_F1_SEAfixed.svg", plot = image, width = 16, height = 5)

```
Make heatmap out of that
```{r, fig.height=6}

# Step 1: Calculate mean and standard deviation of F1 for each combination of tool and comparison_preference
df_summary <- df %>%
  group_by(tool, comparison_preference) %>%
  summarise(mean_F1 = mean(F1), sd_F1 = sd(F1)) %>%
  ungroup()

# Step 2: Create the dot plot with dot size representing SD and color representing mean F1
dot_plot <- ggplot(df_summary, aes(x = tool, y = comparison_preference)) +
  geom_point(aes(size = sd_F1, color = mean_F1)) +  # Dot size for SD and color for mean F1
  scale_size_continuous(name = "Standard Deviation (SD)") +  # Size scale for SD
  scale_color_gradient(low = "blue", high = "red", name = "Mean F1 Score") +  # Color scale for mean F1 score
  theme_minimal() +  # Clean theme
  labs(x = "Tool", y = "Comparison Preference") +  # Label the axes
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability

# Step 3: Display the plot
dot_plot
```
Onlyt take tools of interest and rename
```{r}
df_avg_clean = df_summary %>%
  mutate(tool = recode(tool,
                       "Giotto_delaunay" = "Giotto",
                       "IMCR_histoCAT_delaunay" = "histoCAT",
                       "Misty_juxta30_delaunay" = "Misty",
                       "SEA_bi_100iter_zscore_delaunay" = "SEA"
                       )) %>%
  mutate(comparison_preference = recode(comparison_preference,
                       "ran_vs_self_0.6" = "random vs strong interaction 0-0",
                       "self_0.45_vs_self_0.6" = "weak vs strong interaction 0-0",
                       "ran_vs_self_0.45" = "random vs weak interactino 0-0",
                       )) %>%
  filter(tool %in% c("Giotto", "histoCAT", "Misty", "SEA"))


# Step 2: Create the dot plot with dot size representing SD and color representing mean F1
dot_plot <- ggplot(df_avg_clean, aes(x = tool, y = comparison_preference)) +
  geom_point(aes(size = sd_F1, color = mean_F1)) +  # Dot size for SD and color for mean F1
  scale_size_continuous(name = "Standard Deviation (SD)") +  # Size scale for SD
  scale_color_gradient(low = "blue", high = "red", name = "Mean F1 Score") +  # Color scale for mean F1 score
  theme_minimal() +  # Clean theme
...
df_all_avg_clean = df %>%
  ...
  mutate(comparison_preference = recode(comparison_preference,
                       "ran_vs_self_0.6" = "random vs strong",
                       "self_0.45_vs_self_0.6" = "weak vs strong",
                       "ran_vs_self_0.45" = "random vs weak",
                       )) %>%
  filter(tool %in% c("Giotto", "histoCAT", "Misty", "SEA"))

# Updated plot for publication
image2 <- ggplot(df_all_avg_clean, aes(x = tool, y = F1)) +
  geom_boxplot(aes(fill = tool), color = "black", outlier.size = 1, outlier.shape = 16) +  # Black outline for boxplot and smaller outliers
  facet_grid(. ~ comparison_preference) +  # Facet by comparison_preference
  theme_bw(base_size = 14) +  # Cleaner, more minimalist theme with larger base font
  theme(
    axis.text.x = element_text(size = 12, angle = 45, vjust = 1, hjust = 1),  # Tilt x-axis labels for readability
    ...
    legend.position = "none",  # Remove legend if not needed (or adjust)
    #panel.grid.major = element_blank(),  # Remove major gridlines for clarity
    #panel.grid.minor = element_blank()   # Remove minor gridlines
  ) +
  labs(
    #title = "Comparison of F1 Scores Across Methods",  # Add title
    x = "Method",  # X-axis label
    y = "F1 Score",  # Y-axis label
    #caption = "Each box represents the distribution of F1 scores by method and comparison preference."  # Optional caption
  ) +
  scale_fill_brewer(palette = "Dark2") +  # Use a colorblind-friendly palette
  coord_cartesian(ylim = c(0.2, 1))  # Set limits for y-axis to improve visibility

# Plot
image2
unique(df$tool)
#ggsave("./../../../../Paper_figures/dirSEA_v2/RF_comparison_symmetric_core_tools.svg", plot = image2, device = "svg", width = 6, height = 4)
```
...
vec = sub("X", "", rownames(features[[40]]))
ran = c(0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25)
...
rownames(ranks) = c("0_0", "0_1", "0_2", "0_3", "1_0", "1_1", "1_2", "1_3", "2_0", "2_1", "2_2", "2_3", "3_0", "3_1", "3_2", "3_3")

```
Calcuate 100 cosine simuílarities for each tool for each comparison

expand feature list for giotto
```{r}
giotto_inidces <- which(sapply(features, function(df) nrow(df) == 10))

for (i in giotto_inidces){
  df = t(features[[i]])
  dob = df[, !colnames(df) %in% c("`\\`0_0\\``", "`\\`1_1\\``", "`\\`2_2\\``", "`\\`3_3\\``")]
  #print(dob)
  colnames(dob) = sub("(.*)_(.*)", "\\2_\\1", colnames(dob))
  df2 = t(cbind(df, dob))
...
for (i in giotto_inidces) {
      # Reorder rows of dataset1 using the ordered row names
      features[[i]] <- features[[i]][ordered_cols, , drop = FALSE]
}

for (i in 1:length(features)){
  rownames(features[[i]]) = ordered_cols
}
...
```{r}
df_cosine_avg_clean = cosine_similarities %>%
  mutate(tool = recode(tool,
                       "Giotto_delaunay" = "Giotto",
                       "IMCR_histoCAT_delaunay" = "histoCAT",
                       "Misty_juxta30_delaunay" = "Misty",
                       "SEA_bi_zscorefixed_delaunay" = "SEA",
                       "SEA_100iter_zscorefixed_direction_delaunay" = "dirSEA"
                       )) %>%
  ...
  filter(tool %in% c("Giotto", "histoCAT", "Misty", "SEA", "dirSEA"))

# Updated plot for publication
image2 <- ggplot(df_cosine_avg_clean, aes(x = tool, y = cosine_similarity)) +
  geom_boxplot(aes(fill = tool), color = "black", outlier.size = 1, outlier.shape = 16) +  # Black outline for boxplot and smaller outliers
  facet_grid(. ~ comparison) +  # Facet by comparison_preference
  theme_bw(base_size = 14) +  # Cleaner, more minimalist theme with larger base font
  theme(
    axis.text.x = element_text(size = 12, angle = 45, vjust = 1, hjust = 1),  # Tilt x-axis labels for readability
    axis.text.y = element_text(size = 12),  # Increase size of y-axis text
...
unique(df_cosine_avg_clean$tool)
```
